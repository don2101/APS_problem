...
"""
배운것
1. index out of range 계산 철처히
2. 범위와, direction 계산 정확하게
"""
...
int main(void) {
    ...
    while(t > 0) {
        while(isInside(q+dy[d], p+dx[d])) {
            p = p+dx[d];
            q = q+dy[d];
            --t;
            if(t <= 0) break;
        }
        if(isInside(q+dy[(d+1)%4], p+dx[(d+1)%4])) d = (d+1)%4;
        else if(isInside(q+dy[(d+3)%4], p+dx[(d+3)%4])) d = (d+3)%4;
        else d = (d+2)%4;
    }
