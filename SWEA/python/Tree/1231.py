...
child = [[0, 0] for _ in range(101)]

def inorder(tree, node) :
    if tree[node] :
        inorder(tree, child[node][0])
        print(tree[node], end="")
        inorder(tree, child[node][1])
...
while tc <= t :
    ...
    for _ in range(n) :
        ...
        length = len(nodes)
        if length >= 3 :
            child[int(nodes[0])][0] = int(nodes[2])
        if length == 4 :
            child[int(nodes[0])][1] = int(nodes[3])
        print(child[int(nodes[0])][0], child[int(nodes[0])][1])
