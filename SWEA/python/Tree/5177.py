...
def put(minHeap, data, position) :
    minHeap[position] = data
    cur = position
    parent = position // 2
...
while tc <= t :
    ...
    minHeap = [0 for _ in range(n+1)]
    for i in range(n) :
        put(minHeap, numbers[i], i+1)
    
    ans = getSum(minHeap, n)
